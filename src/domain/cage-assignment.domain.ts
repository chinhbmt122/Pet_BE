...
export class CageAssignmentDomainModel {
    ...
    static create(props: {
        ...
        if (props.expectedCheckOutDate && props.expectedCheckOutDate < checkInDate) {
    ...
    calculateTotalCost(): number {
        const endDate = this._actualCheckOutDate ?? this._expectedCheckOutDate ?? new Date();
    ...
    private calculateDays(startDate: Date, endDate: Date): number {
        ...
        const diffDays = Math.ceil((endDate.getTime() - startDate.getTime()) / oneDay);
    ...
    getDaysRemaining(): number | null {
        if (!this._expectedCheckOutDate || this._status !== CageAssignmentStatus.ACTIVE) {
            return null;
        }
