...
export class PaymentService {
  ...
  private async sendPaymentEmail(
    ...
    try {
      ...
      const ownerName =
        invoice.appointment?.pet?.owner?.fullName || 'Quý khách';
      ...
      if (isSuccess) {
        this.emailService
          .sendPaymentConfirmationEmail(ownerEmail, {
            ownerName,
          ...
          .catch((err) =>
            console.warn('[EMAIL] Payment confirmation failed:', err),
          );
        console.log(`[EMAIL] Payment confirmation sent to ${ownerEmail}`);
      } else {
        this.emailService
          .sendPaymentFailedEmail(ownerEmail, {
            ownerName,
            invoiceNumber: invoice.invoiceNumber,
            amount: Number(payment.amount).toLocaleString('vi-VN') + ' VNĐ',
          ...
          .catch((err) =>
            console.warn('[EMAIL] Payment failed notification error:', err),
          );
        console.log(
